...
void unzipZip(const path &zip_path, const path &unzip_to,
  ...
  std::string cmd = "unzip -q " + zip_path.string() + " -d " + zip_dir.string();
...
void fetchGitArchive(const std::string &git_url, const std::string &branch,
   ...
   */
  std::string url = stripped_git_url + "/archive/refs/heads/" + branch + ".zip";
  /*
  ...
  std::filesystem::path sub_directory_to_extract =
      unzip_to /
      (repo_name + "-" + branch); // this is the directory that github generates
...
int levensteinDistance(std::string aStr, std::string bStr) {
  ...
  for (int i = 1; i <= bLen; i++) {
    ...
    for (int j = 1; j <= aLen; j++) {
      ...
      curr[j] =
          std::min(std::min(curr[j - 1] + 1, prev[j] + 1), prev[j - 1] + cost);
    }
    int *temp = prev;
    prev = curr;
    curr = temp;
