...
bool Tests::Command::testAddPackageToMode(std::string mode_name,
  ...
  auto [failed, inter] =
      init("frate add p " + package_name + " -m " + mode_name + " -l");
  if (failed) {
  ...
  Frate::Utils::error
      << "Failed to add package to mode " + mode_name +
             " : the selected mode either doesn't exist or we couldn't find " +
             package_name
      << std::endl;
