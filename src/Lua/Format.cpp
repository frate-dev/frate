...
namespace Frate::LuaAPI {
  ...
  std::string resolveSpecifier(std::vector<std::string> &specifiers,
                               sol::table table,
                               std::string result = "",
                               size_t index = 0) {
      ...
      if (key.as<std::string>() == specifiers[index]) {
        ...
        if (value.is<std::string>()) {
          ...
          result += resolveSpecifier(
              specifiers, value.as<sol::table>(), result, index + 1);
          // }else if(value.is<sol::object>()){
          //   result += resolveSpecifier(specifiers, value.as<sol::table>(),
          //   result, index+1);
        } else {
          Utils::error
              << "LuaAPI Error: Unrecognized type while resolving specifier"
              << std::endl;
          exit(1);
        }
      }
  ...
  std::string FrateApi::format(const std::string &str,
                               sol::table in_table,
                               sol::this_state current_state) {
    ...
    for (size_t i = 0; i < str.size(); i++) {
      ...
      if (str[i] == '{') {
        ...
        if (next_char != std::string::npos && next_char == '%') {
          ...
          while (str[i] != '%') {
            if (i == str.size()) {
              Utils::error << "LuaAPI Error: while parsing format string no "
                              "closing brace"
                           << std::endl;
              exit(1);
            }
    ...
    for (auto &token : tokens) {
      // If we have a specifier then we start recording it in the specifier
      // vector
