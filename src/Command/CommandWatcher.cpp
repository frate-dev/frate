...
size_t walk(int epoll_fd, std::string path) {
  ...
  for (directory_entry filetoken : recursive_directory_iterator(path) |
           filter([](const directory_entry &entry){return entry.is_directory();})) 
  {
      ...
      int err = inotify_add_watch(temp_fd, filetoken.path().c_str(),
  ...
  watcher(
      [this]() {
        ...
        std::string command = "cmake ./build/ && ./build/make && ./build/" +
                              pro->build_dir + "/" + pro->project_name;
        ...
        if (args->count("args") != 0) {
          ...
          command = "cmake . && make && ./" + pro->build_dir + "/" +
                    pro->project_name + "  " + command_args;
        }
#endif
