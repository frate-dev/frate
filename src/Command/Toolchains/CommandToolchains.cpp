...
namespace Frate::Command::Toolchains {
  ...
  json load() {
    std::filesystem::path path = std::string(std::getenv("HOME")) +
                                 "/.config/" + "frate/" + "toolchains.json";
    return json{};
  }

  bool list(std::shared_ptr<Interface> inter) {
    json data = load();
    for (Command::Toolchain::CompileTarget toolchain :
  ...
  bool add(std::shared_ptr<Interface> inter) {
    ...
    if (!std::filesystem::exists(inter->pro->path / "toolchains/")) {
      try {
        ...
        Utils::error << "Error while creating toolchains directory"
                     << std::endl;
        return false;
      }
    }
  ...
  bool remove(std::string toolchain_name,
              std::shared_ptr<Interface> interface) {
    json data = load();

    for (std::string toolchainProject : interface->pro->toolchains) {
      if (toolchainProject == toolchain_name) {
        std::erase_if(interface->pro->toolchains,
                      [&toolchain_name](std::string &toolchain) {
    ...
    std::filesystem::path tmp_path =
        Utils::copyToTmpPath(interface->pro->path, "toolchains-remove-frate-");
    std::filesystem::remove(interface->pro->path / "toolchains/" /
