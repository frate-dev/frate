...
namespace Frate::Command::License {
  ...
  License searchPrompt(std::string &query) {
    ...
    nlohmann::json github_licenses =
        Utils::fetchJson("https://api.github.com/licenses");
  ...
  void fillLicense(std::shared_ptr<Interface> inter,
                   FullLicense &full_license) {
  ...
  bool set(std::shared_ptr<Interface> inter) {
    ...
    Utils::info << "Writing license to " << inter->pro->path / "LICENSE"
                << std::endl;
