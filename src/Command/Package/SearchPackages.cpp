...
namespace Frate::Command::Packages {
  ...
  std::vector<std::pair<Package, int>>
  calculatePackageScores(std::string &query) {
    ...
    for (Package package : raw_index) {
      ...
      score += (int)((float)Utils::getStringScore(package.name, query) *
                     name_weight);
  ...
  std::vector<Package>
  filterOutBest(std::vector<std::pair<Package, int>> &packages) {
    ...
    float average_score =
        (static_cast<float>(total_score) / static_cast<float>(packages.size()));
  ...
  std::pair<bool, Dependency> searchWithPrompt(std::string &query,
                                               bool latest) {
    auto results = calculatePackageScores(query);
    if (results.empty()) {
    ...
    List package_list{};
    package_list.Numbered().ReverseIndexed();
    for (auto &result : results) {
      package_list.pushBack(
          ListItem(result.first.name + " (" + result.first.git + ")",
                   result.first.description));
