...
namespace Frate::Command::Packages {
  ...
  bool removePackagesByMode(std::shared_ptr<Interface> inter,
                            std::string mode) {
...
Usage remove dep:
   ...
   cmake remove dep [args] 
    ...
    if (inter->args->count("mode") != 0) {
      return removePackagesByMode(
          inter, inter->args->operator[]("mode").as<std::string>());
    }

    std::vector<std::string> name_to_remove =
        inter->args->operator[]("args").as<std::vector<std::string>>();
