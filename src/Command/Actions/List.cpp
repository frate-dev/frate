...
namespace Frate::Command::List {
  ...
  std::vector<Handler> handlers(std::shared_ptr<Interface> inter) {
    return {
        Handler{.aliases = {"modes", "m"},
                .docs = "List modes",
                .callback = &Modes::list,
                .requires_project = true},
        Handler{
            .aliases = {"servers", "remote-servers"},
            .docs = "List remote servers",
            .callback = &RemoteServers::list,
        },
        Handler{.aliases = {"installed-packages", "packages", "p"},
                .flags = {"-m", "--mode"},
                .docs = "List installed packages",
                .callback = &Command::Packages::list,
                .requires_project = true},
        Handler{.aliases = {"flags", "f"},
                .docs = "List flags",
                .callback = &Flags::list,
                .requires_project = true},
        Handler{.aliases = {"libraries", "lib", "l"},
                .docs = "List flags",
                .callback = &Library::list,
                .requires_project = true},
        Handler{.aliases = {"authors", "a"},
                .docs = "List authors",
                .callback = &Author::list,
                .requires_project = true},
        Handler{
            .aliases = {"available-targets", "at"},
            .docs = "List available targets",
            .callback = &Toolchains::list,
        },
        Handler{.aliases = {"licenses", "lc"},
  ...
  bool run(std::shared_ptr<Interface> inter) {
    ...
    if (inter->args->operator[]("subcommand").count() > 0) {
      ...
      std::cout << termcolor::bright_red << "No subcommand provided"
                << std::endl;
