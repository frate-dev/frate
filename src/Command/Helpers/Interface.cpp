...
namespace Frate::Command {
  ...
  bool Interface::parse() {
    try {
      this->args = std::make_shared<cxxopts::ParseResult>(
  ...
  bool Interface::InitHeader() {
    try {
      this->options = std::make_shared<cxxopts::Options>(
          "Frate",
          "A CMake project generator, we suffer so you don't have to!");
...
//     TargetPackageHandler add_handler(inter);
    ...
    inter->commands.push_back({.aliases = {"new", "n"},
    ...
    inter->commands.push_back({.aliases = {"help", "h"},
                               .flags = {}, // TODO: Add flags
                               .docs = "Display help",
                               .callback = &Help::run,
                               .requires_project = false});

    inter->commands.push_back({.aliases = {"nuke"},
                               .flags = {}, // TODO: Add flags
                               .docs = "Deletes the entire project",
                               .callback = &FTP::run,
                               .requires_project = false});

    inter->commands.push_back({.aliases = {"add"},
                               .flags = {}, // TODO: Add flags
                               .subcommands = Add::handlers(inter),
                               .docs = "add sub command",
                               .callback = &Add::run});

    inter->commands.push_back({.aliases = {"set"},
                               .flags = {}, // TODO: Add flags
                               .subcommands = Set::handlers(inter),
                               .docs = "set sub command",
                               .callback = &Set::run});

    inter->commands.push_back({.aliases = {"search"},
                               .flags = {}, // TODO: Add flags
                               .subcommands = Search::handlers(inter),
                               .docs = "search sub command",
                               .callback = &Search::run,
                               .requires_project = false});
    inter->commands.push_back({.aliases = {"list", "ls"},
                               .flags = {}, // TODO: Add flags
    ...
    inter->commands.push_back({.aliases = {"remove", "rm"},
                               .flags = {}, // TODO: Add flags
                               .subcommands = Remove::handlers(inter),
                               .docs = "remove sub command",
                               .callback = &Remove::run});

    inter->commands.push_back({.aliases = {"update"},
                               .flags = {}, // TODO: Add flags
    ...
    inter->commands.push_back({.aliases = {"clean"},
    ...
    inter->commands.push_back(
        {.aliases = {"build"},
         ...
         .callback = &Build::run});
    inter->commands.push_back({.aliases = {"completions"},
                               .flags = {}, // TODO: Add flags
                               .docs = "completions sub command",
                               .callback = &Completions::ZshCompletions,
                               .implemented = false});
    inter->commands.push_back({.aliases = {"watch"},
                               .flags = {}, // TODO: Add flags
  ...
  void renderFlags(std::vector<std::string> flags) {
    ...
    for (std::string flag : flags) {
      std::cout << " " << termcolor::bold << termcolor::magenta << flag
                << termcolor::reset;
    }
    std::cout << " ]";
  }
  ...
  bool runCommand(std::shared_ptr<Interface> inter, std::string command,
    ...
    for (Handler handler : handlers) {
      for (std::string alias : handler.aliases) {
        if (alias == command) {
          ...
          //   Utils::error << "Error: Project not found and command: " <<
          //   command << " requires a project" << std::endl; return false;
          // }
          if (!handler.callback(inter)) {
    ...
    std::cout << termcolor::red << "Error: Subcommand not found: " << command
              << termcolor::reset << std::endl;
    return false;
  }

  void Interface::getHelpString(Handler &handler) {
    std::cout << termcolor::bold << termcolor::yellow << handler.aliases[0]
              << termcolor::reset;
  ...
  void Interface::getHelpString(std::string name,
                                std::vector<Handler> &handlers,
                                bool is_subcommand) {
    size_t index = 0;
    for (Handler handler : handlers) {
      ...
      for (std::string alias : handler.aliases) {
        ...
        std::cout << termcolor::bold << termcolor::yellow << alias
                  << termcolor::reset;
